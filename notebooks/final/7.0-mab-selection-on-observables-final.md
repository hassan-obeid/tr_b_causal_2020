...
The purpose of this notebook is to illustrate an example of the workflow outlined in [Brathwaite and Walker (2017)](https://arxiv.org/abs/1706.07502). This simple application aims at highlighting the importance of causal structure in affecting causal estimates of interests. This notebook achieves this objective by using the dataset and the MNL utility specification from [Brathwaite and Walker (2016)](https://arxiv.org/abs/1606.05900).
 ...
 * An "independent" causal graph: All explanatory variables are independent and do not affect each other. Any change in any variable would not result in changes in other variables.
 * A "realistic" causal graph: The structure of this causal graph was assumed in a way that allows for variables to affect each other.
...
We simulate data based on the assumed structure of the causal diagrams. This process follows this logic:
 * We fit a statistical distribution for all the nodes without any parents in the causal diagrams
 * We fit a regression between the related explanatory variables
 
Based on these fitted distributions and regressions, we can then simulate data for each of the alternatives in our specified model.
