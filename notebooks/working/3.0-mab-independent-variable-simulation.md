...
def altSpecDist(data_long, alt_id_col, alt_spec, var_types, alt_name_dic, cont_dists):
...
# Loop around the different available alternatives
def tripSpecDist(data_long, obs_id_col, trip_spec, cont_dists):
...
def isCategorical(variable):
    return variable == 'categorical'

## Simulate categorical variable
def SimCategorical(variable, size):
    data_sim = np.random.choice(a=variable['parameters'][0],
                                p=variable['parameters'][1],
                                size=size)
    return data_sim
...
def SimContinuous(variable, size):
    dist = getattr(scipy.stats, variable['distribution'])
    data_sim = dist.rvs(*variable['parameters'], size=size)
    return data_sim
```
