...
def get_categorical_dist(var, var_val, alt_name=None):
    ...
    array_bincount = np.bincount(var_val)
    probs = array_bincount / len(var_val)
    categ_dict[var_name]['parameters'] =\ 
...
def sim_from_distribution(distribution, parameters, size):
    """
    Funtion to simulate data of size N based type of dist
    and the distribution parameters.
    """
    if is_categorical(distribution):
        sim_array = sim_categorical(parameters, size)
    elif is_constant(distribution):
        sim_array = sim_constant(parameters)
    else:
        sim_array = sim_continuous(distribution, parameters, size)
    return sim_array
...
def sim_df(params_dict, size=1000):
    ...
    for column in list(params_dict.keys()):
        ...
        sim_df[column] = sim_from_distribution(distribution,
                                               parameters,
                                               size)
    return sim_df
```
