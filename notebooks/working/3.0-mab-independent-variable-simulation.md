...
def SimulateAvailability(data_long, sim_data, obs_id_col, alt_name_dict):
    ...
    AV_columns=['AV_'+alt_name_dict[i] for i in alt_name_dict.keys()]
    AV_Df = pd.DataFrame(av_sim, columns=AV_columns)
    Sim_DF_AV = pd.concat([sim_data, AV_Df], axis = 1, sort=False)
    return Sim_DF_AV
```
...
```python
SimulateAvailability(bike_data_long, sim_data=sim_data, obs_id_col=observation_id_col, alt_name_dict=alternative_name_dict)
```
