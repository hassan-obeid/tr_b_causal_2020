...
## Check if constant
def isConstant(params_dict, column):
    return params_dict[column]['distribution'] == 'constant'

## Check if categorical
def isCategorical(params_dict, column):
    return params_dict[column]['distribution'] == 'categorical'

## Simulate categorical variable
def SimCategorical(params_dict, column):
...
def SimConstant(params_dict, column):
    data_sim = params_dict[column]['parameters'][0]
    Sim_Df[column] = data_sim
    return

## Simulate continuous data
def SimContinuous(params_dict, column):
...
## Loop function
def SimFromDict(params_dict, SimDf):
    for column in list(params_dict.keys()):
        if isCategorical(params_dict, column):
            SimConstant(params_dict, column)
        elif isConstant(params_dict, column):
            SimConstant(params_dict, column)
        else:
            SimContinuous(params_dict, column)
    return Sim_Df
```
