...
def get_constant_dist(var, var_val, alt_name=None):
    ...
    alt_specific_var_name =\
        var if alt_name None else get_alt_specific_variable_name(var, alt_name)
    constant_dict[alt_specific_var_name]['distribution'] = 'constant'
    constant_dict[alt_specific_var_name]['parameters'] = var_val.unique()
    return constant_dict
...
def get_categorical_dist(var, var_val, alt_name=None):
    ...
    alt_specific_var_name =\
        var if alt_name None else get_alt_specific_variable_name(var, alt_name)
    categ_dict[alt_specific_var_name]['distribution'] = 'categorical'
    np_array_range = np.arange(var_val.max()+1)
    array_bincount = np.bincount(var_val)
    probs = array_bincount / len(var_val)
    categ_dict[alt_specific_var_name]['parameters'] =\ 
        [np_array_range, probs]
    return categ_dict

def get_continuous_dist(var, var_val, alt_name=None, cont_dists):
    cont_dict = defaultdict(dict)
    ...
    alt_specific_var_name =\
        var if alt_name None else get_alt_specific_variable_name(var, alt_name)
    cont_dict[alt_specific_var_name]['distribution'] =\ 
        list(BestDict.items())[0][0]
    cont_dict[alt_specific_var_name]['parameters'] =\ 
...
# Code to get alternative specific variable name
...
def alt_spec_dist(data_long, alt_id_col, alt_spec, var_types, alt_name_dic, cont_dists):
...
def trip_spec_dist(data_long, obs_id_col, trip_spec, var_types, cont_dists):
    ...
    for trip_var in trip_spec:
        ...
        trip_spec_var_dic = get_distribution_dicts(trip_var,
                                                  var_type,
                                                  var_val,
                                                  cont_dists)
...
##############################################################
# Defined function
def find_long_data_dist(data_long,
                        alt_id_col,
                        obs_id_col,
                        alt_spec,
                        alt_name_dic,
...
```python
### Functions to repace within the SimDf Function
## Check if constant
def is_constant(distribution):
    return distribution == 'constant'

## Check if categorical
def is_categorical(distribution):
    return distribution == 'categorical'

## Simulate categorical variable
def sim_categorical(parameters, size):
    data_sim = np.random.choice(a=parameters[0],
                                p=parameters[1],
                                size=size)
    return data_sim

## Simulate constant data
def sim_constant(parameters):
    data_sim = parameters[0]
    return data_sim

## Simulate continuous data
def sim_continuous(distribution, parameters, size):
    dist = getattr(scipy.stats, distribution)
    data_sim = dist.rvs(*parameters, size=size)
    return data_sim

## Function to simulate based on the
## parameters dictionary
def sim_df(params_dict, size=1000):
    """
    ...
    for column in list(params_dict.keys()):
        distribution = params_dict[column]['distribution']
        parameters = params_dict[column]['parameters']
        if is_categorical(distribution):
            Sim_Df[column] = sim_categorical(parameters, size)
        elif is_constant(distribution):
            Sim_Df[column] = sim_constant(parameters)
        else:
            Sim_Df[column] = sim_continuous(distribution, parameters, size)
    return Sim_Df
```
...
def sim_fake_choice_col(AV_matrix, sim_col='sim_choice'):
    ...
    #fake_choice = [random.choice(np.nonzero(a == 1)[0]) + 1 for a in np.array(AV_matrix)] -- Depracated
...
def simulate_availability(data_long, sim_size=1000, obs_id_col, alt_name_dict):
    ...
    # Create Series to hold number of available alternatives
    ...
    # be needed when converting to long data -- this will
    # be moved to a different column
    fake_choice_df = sim_fake_choice_vol(AV_Df)
...
# format data
bike_data_params = find_long_data_dist(data_long=bike_data_long,
...
sim_data = sim_df(params_dict=bike_data_params, size=2000)
```
...
sim_data_availability = simulate_availability(bike_data_long,
                                     sim_size=2000,
                                     obs_id_col=observation_id_col,
                                     alt_name_dict=alternative_name_dict)
wide_sim_data = pd.concat([sim_data, sim_data_availaibility],
                          axis=1, sort=False)
```
...
for size, number in zip(simulation_size, sim_number):
    ...
    # Simulate data from the params_dict
    sim_data = sim_df(params_dict=bike_data_params,
                           size=size)

    # Simulate availability based on availability 
    # from long data and addd a fake choice column
    sim_data_availability = simulate_availability(data_long=bike_data_long, 
                                         sim_size=size, 
                                         obs_id_col=observation_id_col, 
                                         alt_name_dict=alternative_name_dict)
...
for par in sim_estimated_par.columns:
    plot_params(sim_estimated_par[par],mnl_model, (10,6))
```
...
non_outliers_matrix = np.empty((0, sim_data.shape[0]))
...
for col in columns:
    non_outliers = find_outliers(sim_data[col])
...
sim_data_clean = sim_data[non_outlier_obs]
sim_data_clean.reset_index(drop=True, inplace=True)
```
...
sns.distplot(sim_data['total_travel_time_drive_alone'],label ='Simulated Data')
sns.distplot(sim_data_clean['total_travel_time_drive_alone'],label ='Simulated Data without outliers')
plt.legend()
```
...
sns.distplot(sim_data['total_travel_time_shared_2'],label ='Simulated Data')
sns.distplot(sim_data_clean['total_travel_time_shared_2'],label ='Simulated Data without outliers')
plt.legend()
```
...
sns.distplot(sim_data['total_travel_time_shared_3p'],label ='Simulated Data')
sns.distplot(sim_data_clean['total_travel_time_shared_3p'],label ='Simulated Data without outliers')
plt.legend()
```
