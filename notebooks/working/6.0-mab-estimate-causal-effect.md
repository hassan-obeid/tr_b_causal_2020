...
## 4.2. Based on Causal Graph and Estimated Model from Simulated

```python
long_sim_data_causal = copy.deepcopy(long_sim_data)
```

### 4.2.1. Perturb X and children nodes
...
## Based on True Causal Graph and Original Estimated Model
...
sns.distplot(true_effect, label ='True Effect')
sns.distplot(naive_effect,label ='Naive Effect')
plt.legend()
```
...
plt.figure(figsize=(15,10))
sns.distplot(true_effect, label ='True Effect')
sns.distplot(estimated_effect,label ='Estimated effect')
plt.legend()
```
