...
For our illustrative purposes, we focus on the following car-centric modes and will not further present details about other modes:
...
The systematic utility equations of the adopted multinomial logit model for the car-centric modes are specified as follows:
\begin{equation*}
   \begin{aligned}
...
Similarly, Figure \ref{fig:DA_causal_2} through \ref{fig:SR3_causal_2} illustrate the causal graphs with ``dependent'' explanatory variables.
This DAG represents a scenario where an analyst considers the relationships between all variables and modifies all children nodes of the variable directly impacted by the proposed intervention.
