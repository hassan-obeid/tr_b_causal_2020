...
Finally, natural experiments are instances where our system is subject to intervention, though not necessarily randomly.
Imagine a rule that affects some individuals but not others.
...
One type of causal discovery algorithm takes a causal graph with unresolved ambiguities, and it outputs an experimentation plan whose resulting data will permit resolution of the causal graph.
For algorithms of this kind, see work by \citet{ghassami_2018_budgeted}, the work cited therein, and related literature.
The other type of algorithm takes whatever experimental and observational data one already has, and it tries to best combine these sources of information to construct a causal graph that is valid for all datasets.
