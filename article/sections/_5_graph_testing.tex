...
This recasts our original test statistic of $R^2$ as a function of our permutation of $Y$ and our imputation of $Z$.
Note that observed $Y$ is the permutation of $Y$ that uses the original ordering of the data.
We'll now compute the p-value of this test statistic by marginalizing over the permutations and imputations.
...
For reference, this is the p-value for test statistics (or discrepancies) defined in \citet[Eq. 7]{gelman_1996_posterior}.

Lastly, because our test statistic is itself a random variable that depends on the imputed values of $Z$, we change our visualization method.
Instead of plotting a single line versus a distribution, we now plot two distributions against each other.
...
Then, we plot the distribution of ``imputed'' test statistics as a reference.
In both distributions, we have one value per imputed vector $Z$.
However, for the distribution of ``imputed'' test statistics, we marginalize over permutations of $Y$ since this distribution represents the null hypothesis of conditional or marginal independence.
