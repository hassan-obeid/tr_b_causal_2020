...
The techniques presented in this section have focused on testing independence assertions using one's dataset.
While this approach may be the most accessible of strategies for testing one's causal graph, they are not the only strategies.
...
Alternatively, there are additional quantitative tests of one's causal graph that have not been explored in this section.

For instance, a causal graph makes assumptions about the number of independent variables in one's dataset: i.e., the nodes in one's graph that do not have parent-nodes.
...
For more information on estimating the intrinsic dimension of a dataset, see \citet{}.
