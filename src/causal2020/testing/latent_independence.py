...
def compute_pvalue(
    obs_statistic: Union[Number, np.ndarray], reference_statistics: np.ndarray
) -> float:
    """
...
def compute_predictive_independence_test_values(
    ...
    for i in iterable:
        ...
        current_augmented_obs = np.concatenate(
            (obs_sample, current_sim_z[:, None]), axis=1
        )
        current_seed = seed + i
        ...
        sampled_pvals[i] = compute_pvalue(*oi.computed_vs_obs_r2(*sim_args))

        obs_pvals[i] = compute_pvalue(
            *oi.computed_vs_obs_r2(*augmented_obs_args)
        )
...
def visualize_predictive_cit_results(
    ...
    if verbose:
        msg = "The p-value of the predictive, permutation C.I.T. is {:.2f}."
...
def plot_simulated_vs_observed_cdf(
    ...
    sim_cdf._plot_single_cdf_on_axis(
        obs_array, ax, color=obs_color, label="Observed", alpha=1.0
    )

    # Label the plot axes
    ax.set_xlabel("{}".format(x_label), fontsize=12)
    ax.set_ylabel(
        "Cumulative\nDensity\nFunction", rotation=0, labelpad=40, fontsize=12
    )
